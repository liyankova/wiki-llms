---
url: https://nix.dev/manual/nix/2.32/language/syntax
title: Syntax and semantics - Nix 2.32.2 Reference Manual
source_domain: nix.dev
---

# Syntax and semantics - Nix 2.32.2 Reference Manual

# [Language Constructs](https://nix.dev/manual/nix/2.32/language/syntax#language-constructs)

This section covers syntax and semantics of the Nix language.

## [Basic Literals](https://nix.dev/manual/nix/2.32/language/syntax#basic-literals)

### [String](https://nix.dev/manual/nix/2.32/language/syntax#string-literal)

See [String literals](https://nix.dev/manual/nix/2.32/language/string-literals).

### [Number](https://nix.dev/manual/nix/2.32/language/syntax#number-literal)

Numbers, which can be *integers* (like `123`) or *floating point*
(like `123.43` or `.27e13`).

Integers in the Nix language are 64-bit [two's complement](https://en.wikipedia.org/wiki/Two%27s_complement) signed integers, with a range of -9223372036854775808 to 9223372036854775807, inclusive.
...
See [arithmetic](https://nix.dev/manual/nix/2.32/language/operators#arithmetic) and [comparison](https://nix.dev/manual/nix/2.32/language/operators#comparison) operators for semantics.

### [Path](https://nix.dev/manual/nix/2.32/language/syntax#path-literal)
...
Path literals are resolved relative to their [base directory](https://nix.dev/manual/nix/2.32/glossary#gloss-base-directory).
...
Path literals that start with `~` are not allowed in [pure](https://nix.dev/manual/nix/2.32/command-ref/conf-file#conf-pure-eval) evaluation.

Path literals can also include [string interpolation], besides being [interpolated into other expressions](https://nix.dev/manual/nix/2.32/language/string-interpolation#interpolated-expressions).
...
[Lookup path](https://nix.dev/manual/nix/2.32/language/constructs/lookup-path) literals such as `<nixpkgs>` also resolve to path values.

## [List](https://nix.dev/manual/nix/2.32/language/syntax#list-literal)
...
Elements in a list can be accessed using [`builtins.elemAt`](https://nix.dev/manual/nix/2.32/language/builtins#builtins-elemAt).

## [Attribute Set](https://nix.dev/manual/nix/2.32/language/syntax#attrs-literal)
...
An attribute name is a string without context, and is denoted by a [name](https://nix.dev/manual/nix/2.32/language/identifiers#names) (an [identifier](https://nix.dev/manual/nix/2.32/language/identifiers#identifiers) or [string literal](https://nix.dev/manual/nix/2.32/language/string-literals)).
...
An attribute path is a dot-separated list of [names](https://nix.dev/manual/nix/2.32/language/identifiers#names).
...
Attribute names can also be set implicitly by using the [`inherit` keyword](https://nix.dev/manual/nix/2.32/language/syntax#inheriting-attributes).
...
Attributes can be accessed with the [`.` operator](https://nix.dev/manual/nix/2.32/language/operators#attribute-selection).
...
A set that has a [`__functor`](https://nix.dev/manual/nix/2.32/language/syntax#attr-__functor) attribute whose value is callable (i.e. is
...
## [Recursive sets](https://nix.dev/manual/nix/2.32/language/syntax#recursive-sets)

Recursive sets are like normal [attribute sets](https://nix.dev/manual/nix/2.32/language/types#attribute-set), but the attributes can refer to each other.
...
## [Inheriting attributes](https://nix.dev/manual/nix/2.32/language/syntax#inheriting-attributes)

When defining an [attribute set](https://nix.dev/manual/nix/2.32/language/types#attribute-set) or in a [let-expression](https://nix.dev/manual/nix/2.32/language/syntax#let-expressions) it is often convenient to copy variables from the surrounding lexical scope (e.g., when you want to propagate attributes).
...
## [Functions](https://nix.dev/manual/nix/2.32/language/syntax#functions)
...
## [Conditionals](https://nix.dev/manual/nix/2.32/language/syntax#conditionals)
...
## [Assertions](https://nix.dev/manual/nix/2.32/language/syntax#assertions)
...
## [With-expressions](https://nix.dev/manual/nix/2.32/language/syntax#with-expressions)
...
## [Comments](https://nix.dev/manual/nix/2.32/language/syntax#comments)
