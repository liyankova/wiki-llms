---
url: https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address
title: Content-addressing derivation outputs - Nix 2.32.2 Reference Manual
source_domain: nix.dev
---

# Content-addressing derivation outputs - Nix 2.32.2 Reference Manual

# [Content-addressing derivation outputs](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#content-addressing-derivation-outputs)
...
The possible choices of *method* are described in the [section on content-addressing store objects](https://nix.dev/manual/nix/2.32/store/store-object/content-address).
...
## [Fixed-output content-addressing](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#fixed)

In this case the content address of the *fixed* in advanced by the derivation itself.
In other words, when the derivation has finished [building](https://nix.dev/manual/nix/2.32/store/building), and the provisional output' content-address is computed as part of the process to turn it into a *bona fide* store object, the calculated content address must much that given in the derivation, or the build of that derivation will be deemed a failure.
...
### [Design rationale](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#design-rationale)
...
Unlike a regular derivation, the [builder](https://nix.dev/manual/nix/2.32/store/derivation/#builder) executable of a derivation that produced fixed outputs has access to the network.
...
If a `fetchurl` derivation's outputs were [input-addressed](https://nix.dev/manual/nix/2.32/store/derivation/outputs/input-address), the output paths of the derivation and of *all derivations depending on it* would change.
...
> This is why the rationale above contrasts it with [input addressing](https://nix.dev/manual/nix/2.32/store/derivation/outputs/input-address).

## [(Floating) Content-Addressing](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#floating)

> **Warning**
> This is part of an [experimental feature](https://nix.dev/manual/nix/2.32/development/experimental-features).
>
> To use this type of output addressing, you must enable the
> [`ca-derivations`](https://nix.dev/manual/nix/2.32/development/experimental-features#xp-feature-ca-derivations) experimental feature.
...
### [Purity](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#purity)

Because the derivation output is not fixed (just like with [input addressing](https://nix.dev/manual/nix/2.32/store/derivation/outputs/input-address)), the [builder](https://nix.dev/manual/nix/2.32/store/derivation/#builder) is not given any impure capabilities [^purity].
...
This is provisionally supported as part of the [`impure-derivations`](https://nix.dev/manual/nix/2.32/development/experimental-features#xp-feature-impure-derivations) experimental feature.

### [Compatibility negotiation](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#compatibility-negotiation)

Any derivation producing a floating content-addressed output implicitly requires the `ca-derivations` [system feature](https://nix.dev/manual/nix/2.32/command-ref/conf-file#conf-system-features).
...
### [Determinism](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#determinism)

In the earlier [discussion of how self-references are handled when content-addressing store objects](https://nix.dev/manual/nix/2.32/store/store-object/content-address#self-references), it was pointed out that methods of producing store objects ought to be deterministic regardless of the choice of provisional store path.
...
The builder the data it produces based on it in arbitrary ways, but this gets us closer to [input addressing](https://nix.dev/manual/nix/2.32/store/derivation/outputs/input-address).
...
### [Floating versus Fixed](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#floating-versus-fixed)
...
> **Design Note**
>
> In a future world where floating content-addressing is also stable, we in principle no longer need separate [fixed](https://nix.dev/manual/nix/2.32/store/derivation/outputs/content-address#fixed) content-addressing.
