...
title: Flakes — nix.dev  documentation
source_domain: nix.dev
---

# Flakes — nix.dev  documentation

[Skip to main content](https://nix.dev/concepts/flakes#main-content)
...
* [.md](https://nix.dev/_sources/concepts/flakes.md "Download source file")

# Flakes

# Flakes[#](https://nix.dev/concepts/flakes#flakes "Link to this heading")

What is usually referred to as “flakes” is:

* A policy for managing dependencies between [Nix expressions](https://nix.dev/reference/glossary#term-Nix-expression).
* An [experimental feature](https://nix.dev/manual/nix/stable/contributing/experimental-features) in Nix, implementing that policy and supporting functionality.

## What are flakes?[#](https://nix.dev/concepts/flakes#what-are-flakes "Link to this heading")
...
1. A `flake.nix` file enforces a [schema](https://nix.dev/manual/nix/stable/command-ref/new-cli/nix3-flake.html#flake-format), where:

   * Other flakes are referenced as dependencies providing [Nix language](https://nix.dev/reference/glossary#term-Nix-language) code or other files.
   * The values produced by the [Nix expression](https://nix.dev/reference/glossary#term-Nix-expression)s in `flake.nix` are structured according to pre-defined use cases.
2. References to other flakes can be specified using a dedicated [URL-like syntax](https://nix.dev/manual/nix/stable/command-ref/new-cli/nix3-flake.html#flake-references).
   A [flake registry](https://nix.dev/manual/nix/stable/command-ref/new-cli/nix3-registry.html) allows using symbolic identifiers for further brevity.
   References can be automatically locked to their current specific version and later updated programmatically.
3. A [new command line interface](https://nix.dev/manual/nix/stable/command-ref/new-cli/nix.html), implemented as a separate experimental feature, leverages flakes by accepting flake references in order to build, run, or deploy software defined as a flake.

Nix handles flakes differently than regular [Nix files](https://nix.dev/reference/glossary#term-Nix-file) in the following ways:
...
## Why are flakes controversial?[#](https://nix.dev/concepts/flakes#why-are-flakes-controversial "Link to this heading")

[Flakes](https://nix.dev/concepts/flakes#flakes-definition) were inspired by [Shea Levy’s NixCon 2018 talk](https://www.youtube.com/watch?v=DHOLjsyXPtM), formally proposed in [RFC 49](https://github.com/NixOS/rfcs/pull/49), and have been in development since 2019.
Nix introduced the implementation as its first [experimental feature](https://nix.dev/manual/nix/stable/contributing/experimental-features) in 2021.
...
* Copying sources to the Nix store prior to evaluation adds a [significant performance penalty](https://github.com/NixOS/nix/issues/3121), especially for large repositories such as [Nixpkgs](https://nix.dev/reference/glossary#term-Nixpkgs).
...
[Nixpkgs](https://nix.dev/reference/glossary#term-Nixpkgs) as a contrasting example, while featuring a `flake.nix` for compatibility, does not depend on Nix experimental features in its code base.

## Should I use flakes in my project?[#](https://nix.dev/concepts/flakes#should-i-use-flakes-in-my-project "Link to this heading")

You have to judge for yourself based on your needs.

[Flakes](https://nix.dev/concepts/flakes#flakes-definition) emphasize reproducible artifacts and convenience for their consumers, while classic Nix tools center around composable building blocks and customisation options for developers.
...
* The constraints imposed on flakes strengthen reproducibility by default, and enable some performance improvements when interacting with a large Nix package repository like [Nixpkgs](https://nix.dev/reference/glossary#term-Nixpkgs).
* Flake references allow for easier handling of version upgrades for existing packages or project dependencies.
* The [flake schema](https://nix.dev/manual/nix/stable/command-ref/new-cli/nix3-flake.html#flake-format) helps with composing Nix projects from multiple sources in an orderly fashion.

At the same time, flakes have [fundamental architectural issues](https://nix.dev/concepts/flakes#flakes-controversy) and a number of [problems with the implementation](https://github.com/NixOS/nix/issues?q=is%3Aissue+is%3Aopen+label%3Aflakes+sort%3Areactions-%2B1-desc), and there is no coordinated effort to resolve them systematically.
...
In particular, the process of adding software to [Nixpkgs](https://nix.dev/reference/glossary#term-Nixpkgs) or maintaining [NixOS](https://nix.dev/reference/glossary#term-NixOS) modules and configurations is not affected by flakes at all.
There is also no evidence that flakes could help solve the scalability challenges of either.

Finally, there are downsides to relying on [experimental features](https://nix.dev/manual/nix/stable/contributing/experimental-features) in general:
...
## Further reading[#](https://nix.dev/concepts/flakes#further-reading "Link to this heading")
