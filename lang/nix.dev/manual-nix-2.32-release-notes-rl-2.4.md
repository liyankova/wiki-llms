---
url: https://nix.dev/manual/nix/2.32/release-notes/rl-2.4
title: Release 2.4 (2021-11-01) - Nix 2.32.2 Reference Manual
source_domain: nix.dev
---

# Release 2.4 (2021-11-01) - Nix 2.32.2 Reference Manual

# [Release 2.4 (2021-11-01)](https://nix.dev/manual/nix/2.32/release-notes/rl-2.4#release-24-2021-11-01)
...
## [Highlights](https://nix.dev/manual/nix/2.32/release-notes/rl-2.4#highlights)
...
* The **`nix` command** has seen a lot of work and is now almost at
  ...
  use](https://nix.dev/manual/nix/2.32/development/cli-guideline) than the old CLI. It is still
...
* **Flakes** are a new format to package Nix-based projects in a more
  ...
  manual page](https://nix.dev/manual/nix/2.32/command-ref/new-cli/nix3-flake).
...
## [Other features](https://nix.dev/manual/nix/2.32/release-notes/rl-2.4#other-features)
...
* There are many new `nix` subcommands:
  ...
  + Various other replacements for the old CLI, e.g. `nix store gc`,
    `nix store delete`, `nix store repair`, `nix nar dump-path`, `nix store prefetch-file`, `nix store prefetch-tarball`, `nix key` and
    `nix daemon`.
...
* **Languages changes**: the only new language feature is that you can
  now have antiquotations in paths, e.g. `./${foo}` instead of `./. + foo`.
...
* The priority of substituters can now be overridden using the
  `priority` substituter setting (e.g. `--substituters 'http://cache.nixos.org?priority=100 daemon?priority=10'`).
...
## [Incompatible changes](https://nix.dev/manual/nix/2.32/release-notes/rl-2.4#incompatible-changes)
...
## [Contributors](https://nix.dev/manual/nix/2.32/release-notes/rl-2.4#contributors)
